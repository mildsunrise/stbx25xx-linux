/*
 * arch/powerpc/platforms/40x/ip350.c
 *
 * Architecture- / platform-specific boot-time initialization code for
 * IBM PowerPC 4xx based AB IPBox 350. Adapted from original
...
static __initdata struct of_device_id ip350_of_bus[] = 
{
  { .compatible = "ibm,plb3", },
  { .compatible = "ibm,opb", },
  { .compatible = "ibm,ebc", },
...
static void ip350_dm9000_outblk(void __iomem *reg, void *buf, int count)
{
...
static void ip350_dm9000_inblk(void __iomem *reg, void *buf, int count)
{
...
static struct dm9000_plat_data ip350_dm9000_pdata = 
{
  .flags    = DM9000_PLATF_16BITONLY,
  .inblk    = ip350_dm9000_inblk,
  .outblk   = ip350_dm9000_outblk,
};
...
static struct platform_device dm9000_device = 
{
  ...
  .dev    = 
  { 
    .platform_data  = &ip350_dm9000_pdata,
  },
};

static struct platform_device *ip350_devs[] __initdata = 
{
  &dm9000_device,
};

static int __init ip350_device_probe(void)
{
  struct device_node *np;
  int irq;

  of_platform_bus_probe(NULL, ip350_of_bus, NULL);
  ...
  platform_add_devices(ip350_devs, ARRAY_SIZE(ip350_devs));

exit:
  return 0;
}

machine_device_initcall(ip350, ip350_device_probe);

static int __init ip350_probe(void)
{
  unsigned long root = of_get_flat_dt_root();

  if (!of_flat_dt_is_compatible(root, "ab,ip350"))
    return 0;

  return 1;
}

define_machine(ip350) {
  .name           = "AB IPBox 350",
  .probe          = ip350_probe,
