/*
 * arch/powerpc/platforms/40x/ip400.c
 *
 * Architecture- / platform-specific boot-time initialization code for
 * IBM PowerPC 4xx based AB IPBox 400. Adapted from original
...
static __initdata struct of_device_id ip400_of_bus[] = 
{
  { .compatible = "ibm,plb3", },
  { .compatible = "ibm,opb", },
  { .compatible = "ibm,ebc", },
...
static void ip400_dm9000_outblk(void __iomem *reg, void *buf, int count)
{
...
static void ip400_dm9000_inblk(void __iomem *reg, void *buf, int count)
{
...
static struct dm9000_plat_data ip400_dm9000_pdata = 
{
  .flags    = DM9000_PLATF_16BITONLY,
  .inblk    = ip400_dm9000_inblk,
  .outblk   = ip400_dm9000_outblk,
};
...
static struct platform_device dm9000_device = 
{
  ...
  .dev    = 
  { 
    .platform_data  = &ip400_dm9000_pdata,
  },
};

static struct platform_device *ip400_devs[] __initdata = 
{
  &dm9000_device,
};

static int __init ip400_device_probe(void)
{
  struct device_node *np;
  int irq;

  of_platform_bus_probe(NULL, ip400_of_bus, NULL);
  ...
  platform_add_devices(ip400_devs, ARRAY_SIZE(ip400_devs));

exit:
  return 0;
}

machine_device_initcall(ip400, ip400_device_probe);

static int __init ip400_probe(void)
{
  unsigned long root = of_get_flat_dt_root();

  if (!of_flat_dt_is_compatible(root, "ab,ip400"))
    return 0;

  return 1;
}

define_machine(ip400) {
  .name           = "AB IPBox 400",
  .probe          = ip400_probe,
